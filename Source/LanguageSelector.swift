    ...
    var body: some View {
        ScrollViewReader { scrollViewReaderProxy in
            List(NumberTranslator.Language.allCases) { listLanguage in
                let selected: Bool = (viewModel.numberTranslator.name(listLanguage) == viewModel.numberTranslator.name(viewModel.currentLanguage))
                ...
                Button {
                    viewModel.currentLanguage = listLanguage
                } label: {
                    HStack(spacing: 0) {
