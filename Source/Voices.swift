...
@Observable class Voices {
    ...
    func readAloud(_ text: String, with voiceIdentifier: String?) {
    ...
    func voiceFor(code: String?) -> String? {
        ...
        for (id, voice) in voiceDict {
            if guess == nil && code == voice.languageCode {
        ...
        for (id, _) in voiceDict {
            if guess == nil {
                guess = id
            }
        }
        if guess != nil {
            UserDefaults.standard.set(guess, forKey: voiceIdentifierKey(code))
        }
        return guess
    }
